<?php
/*
Plugin Name: WPC Fly Cart for WooCommerce (Premium)
...
! defined( 'WOOFC_VERSION' ) && define( 'WOOFC_VERSION', '5.4.8' );
		...
		if ( ! class_exists( 'WPCleverWoofc' ) && class_exists( 'WC_Product' ) ) {
			class WPCleverWoofc {
				...
				function __construct() {
					...
					// ajax pro
					add_action( 'wp_ajax_woofc_get_checkout_form', [ $this, 'get_checkout_form' ] );
					add_action( 'wp_ajax_nopriv_woofc_get_checkout_form', [ $this, 'get_checkout_form' ] );
					add_action( 'wp_ajax_woofc_apply_coupon', [ $this, 'apply_coupon' ] );
					add_action( 'wp_ajax_nopriv_woofc_apply_coupon', [ $this, 'apply_coupon' ] );
					add_action( 'wp_ajax_woofc_remove_coupon', [ $this, 'remove_coupon' ] );
					add_action( 'wp_ajax_nopriv_woofc_remove_coupon', [ $this, 'remove_coupon' ] );
					add_action( 'wp_ajax_woofc_update_shipping_method', [ $this, 'update_shipping_method' ] );
					add_action( 'wp_ajax_nopriv_woofc_update_shipping_method', [ $this, 'update_shipping_method' ] );
					add_action( 'wp_ajax_woofc_calculate_shipping', [ $this, 'calculate_shipping' ] );
					add_action( 'wp_ajax_nopriv_woofc_calculate_shipping', [ $this, 'calculate_shipping' ] );
				...
				function enqueue_scripts() {
					...
					if ( self::get_setting( 'instant_checkout', 'no' ) === 'yes' ) {
						wp_enqueue_script( 'wc-checkout' );
					}

					// js
					wp_enqueue_script( 'woofc-frontend', WOOFC_URI . 'assets/js/frontend.js', [ 'jquery', ], WOOFC_VERSION, true );
					...
					<div class="wpclever_settings_page wrap">
						...
						<div class="wpclever_settings_page_nav">
							<h2 class="nav-tab-wrapper">
                                ...
                                -->
								<a href="<?php echo esc_url( WOOFC_SUPPORT ); ?>" class="nav-tab" target="_blank">
				...
				function remove_coupon() {
					if ( isset( $_POST['coupon_code'] ) ) {
						if ( WC()->cart->remove_coupon( sanitize_text_field( $_POST['coupon_code'] ) ) ) {
				...
				function update_shipping_method() {
					...
					$posted_shipping_methods = isset( $_POST['shipping_methods'] ) ? wc_clean( wp_unslash( $_POST['shipping_methods'] ) ) : [];
				...
				function get_cart_area() {
						...
						foreach ( $items as $cart_item_key => $cart_item ) {
							if ( ! isset( $cart_item['bundled_by'] ) && apply_filters( 'woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
								...
								if ( ( self::get_setting( 'save_for_later', 'yes' ) === 'yes' ) && class_exists( 'WPCleverWoosl' ) ) {
									echo '<span class="woofc-item-save">' . do_shortcode( '[woosl product_id="' . ( $cart_item['variation_id'] ?: $product_id ) . '" cart_item_key="' . $cart_item_key . '" context="woofc"]' ) . '</span>';
								}
					...
					if ( ! empty( $items ) ) {
						...
						if ( ( self::get_setting( 'coupon', 'no' ) === 'yes' ) && WC()->cart->get_cart_contents_count() ) {
							echo apply_filters( 'woofc_above_coupon_content', '' );
							echo '<div class="woofc-coupon">';
							echo '<div class="woofc-coupon-input"><input type="text" name="woofc-coupon-code" class="woofc-coupon-code" placeholder="' . esc_attr( self::localization( 'coupon_code', esc_html__( 'Coupon code', 'woo-fly-cart' ) ) ) . '"/><input type="button" value="' . esc_attr( self::localization( 'coupon_apply', esc_html__( 'Apply', 'woo-fly-cart' ) ) ) . '" class="woofc-coupon-check"/></div>';

							if ( class_exists( 'WPCleverWpccl' ) && ( self::get_setting( 'coupon_listing', 'no' ) === 'yes' ) ) {
								echo '<div class="woofc-coupon-listing">' . do_shortcode( '[wpccl_button]' ) . '</div>';
							}
							...
							foreach ( WC()->cart->get_coupons() as $coupon ) {
								...
								if ( empty( $amount ) && $coupon->get_free_shipping() ) {
									$discount_amount_html = esc_html__( 'Free shipping coupon', 'woo-fly-cart' );
								}

								$discount_amount_html = apply_filters( 'woocommerce_coupon_discount_amount_html', $discount_amount_html, $coupon );
								echo '<div class="woofc-coupon-added" data-code="' . esc_attr( $coupon->get_code() ) . '"><div class="woofc-coupon-added-left"><span class="woofc-coupon-remove">&times;</span> ' . wc_cart_totals_coupon_label( $coupon, false ) . '</div><div class="woofc-coupon-added-right">' . apply_filters( 'woocommerce_cart_totals_coupon_html', $discount_amount_html, $coupon, $discount_amount_html ) . '</div></div>';
							}

							echo '</div>';
							echo apply_filters( 'woofc_below_coupon_content', '' );
						}

						if ( self::get_setting( 'shipping_cost', 'no' ) === 'yes' ) {
							WC()->cart->calculate_shipping();

							if ( self::get_setting( 'shipping_calculator', 'no' ) === 'yes' ) {
								$cost            = '';
								$calculator_text = '';
								...
								if ( ! empty( $packages ) ) {
									...
									if ( $available_methods ) {
										$cost = WC()->cart->get_cart_shipping_total();
									} else {
										$destination = WC()->countries->get_formatted_address( $package['destination'], ', ' );

										if ( ! $destination ) {
											$cost = wp_kses_post( apply_filters( 'woocommerce_shipping_not_enabled_on_cart_html', esc_html__( 'Shipping costs are calculated during checkout.', 'woo-fly-cart' ) ) );
										} else {
											$cost = wp_kses_post( apply_filters( 'woocommerce_cart_no_shipping_available_html', sprintf( esc_html__( 'No shipping options were found for %s.', 'woo-fly-cart' ) . ' ', '<strong>' . esc_html( $destination ) . '</strong>' ) ) );
										}
									}
								}

								echo '<div class="woofc-data"><div class="woofc-data-left"><span class="woofc-shipping">' . self::localization( 'shipping', esc_html__( 'Shipping', 'woo-fly-cart' ) ) . '</span></div><div class="woofc-data-right">' . $cost . '</div></div>';

								echo '<div class="woofc-shipping-area"><div class="woofc-shipping-inner">';
								...
								if ( ! empty( $packages ) ) {
									...
									$destination         = WC()->countries->get_formatted_address( $package['destination'], ', ' );
									$calculated_shipping = ! empty( WC()->customer->has_calculated_shipping() );

									if ( $available_methods ) {
										echo '<div class="woofc-shipping-destination">';

										if ( $destination ) {
											printf( '<span>%1$s</span> <span>%2$s</span>', esc_html__( 'Shipping to:', 'side-cart-woocommerce' ), esc_html( $destination ) );
											$calculator_text = esc_html__( 'Change address', 'woo-fly-cart' );
										} else {
											echo wp_kses_post( apply_filters( 'woocommerce_shipping_estimate_html', esc_html__( 'Shipping options will be updated during checkout.', 'woo-fly-cart' ) ) );
										}

										echo '</div>';

										echo '<ul class="woofc-shipping-methods">';

										foreach ( $available_methods as $method ) {
											echo '<li>';

											if ( 1 < count( $available_methods ) ) {
												printf( '<label><input type="radio" name="woofc_shipping_method[%1$d]" data-index="%1$d" value="%2$s" class="woofc-shipping-method" %3$s />%4$s</label>', $index, esc_attr( $method->id ), checked( $method->id, $chosen_method, false ), wc_cart_totals_shipping_method_label( $method ) ); // WPCS: XSS ok.
											} else {
												printf( '<input type="hidden" name="woofc_shipping_method[%1$d]" data-index="%1$d" value="%2$s" class="woofc-shipping-method" />', $index, esc_attr( $method->id ) ); // WPCS: XSS ok.
											}
										...
										echo '</ul>';
									} elseif ( ! $calculated_shipping || ! $destination ) {
										echo wp_kses_post( apply_filters( 'woocommerce_shipping_may_be_available_html', esc_html__( 'Enter your address to view shipping options.', 'woo-fly-cart' ) ) );
									} else {
										echo wp_kses_post( apply_filters( 'woocommerce_cart_no_shipping_available_html', sprintf( esc_html__( 'No shipping options were found for %s.', 'woo-fly-cart' ) . ' ', '<strong>' . esc_html( $destination ) . '</strong>' ) ) );
										$calculator_text = esc_html__( 'Enter a different address', 'woo-fly-cart' );
									}
								}
								...
								echo '<div class="woofc-data"><div class="woofc-data-left">' . self::localization( 'shipping', esc_html__( 'Shipping', 'woo-fly-cart' ) ) . '</div><div class="woofc-data-right">' . WC()->cart->get_cart_shipping_total() . '</div></div>';
							}
						}
				...
				function footer() {
					...
					if ( self::get_setting( 'auto_show_normal', 'yes' ) === 'yes' ) {
						if ( isset( $_POST['add-to-cart'] ) || isset( $_GET['add-to-cart'] ) || isset( $_REQUEST['product_added_to_cart'] ) ) {
							?>
							<script>
